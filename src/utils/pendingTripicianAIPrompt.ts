 ...
 *
 * TripicianAI cannot answer a guest - `TripicianAIController` is `[Authorize]` at the class
 ...
 * the visitor to sign-up. `TripicianAIPage` replays it on the other side.
 *
 ...
 */
export const PENDING_TRIPICIANAI_PROMPT_KEY = 'tripician:pendingTripicianAIPrompt';
...
function read(): StashedPrompt | null {
  try {
    const raw = sessionStorage.getItem(PENDING_TRIPICIANAI_PROMPT_KEY);
    if (!raw) return null;
    ...
    if (Date.now() - (parsed.ts ?? 0) > MAX_AGE_MS) {
      sessionStorage.removeItem(PENDING_TRIPICIANAI_PROMPT_KEY);
      return null;
    }
    return parsed;
  } catch {
    // Private mode, storage disabled, or a hand-edited value. The visitor just
    // lands on an empty /tripicianai, which is survivable.
    return null;
  }
}
...
export function stashPendingPrompt(prompt: string): void {
  ...
  try {
    sessionStorage.setItem(
      PENDING_TRIPICIANAI_PROMPT_KEY,
 ...
 *
 * The caller is TripicianAIPage's replay effect, which React StrictMode invokes twice
...
export function takePendingPrompt(): string | null {
  const found = read();
  try { sessionStorage.removeItem(PENDING_TRIPICIANAI_PROMPT_KEY); } catch { /* see read() */ }
