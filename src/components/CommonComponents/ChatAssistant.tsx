...
const ChatAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: 'welcome', role: 'assistant', content: 'Hi! I\'m Navia ✨ Your AI travel companion. How can I help you plan your next adventure?' }
  ]);
                ...
                <Box key={m.id} sx={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  ...
                  {m.role === 'user' && (
                    <Box sx={{
                      ...
                      fontSize: '0.82rem', lineHeight: 1.5, color: '#fff',
          ...
          <Box sx={{
            ...
            <IconButton
              ...
              sx={{
                ...
                background: input.trim() ? 'linear-gradient(135deg,#FF385C,#D91A50)' : (isLight ? '#f0f0f0' : 'rgba(255,255,255,0.08)'),
                color: input.trim() ? '#fff' : (isLight ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.3)'),
                transition: 'all 0.2s',
                '&:hover': { background: input.trim() ? 'linear-gradient(135deg,#e02d50,#c01545)' : undefined, transform: input.trim() ? 'scale(1.05)' : undefined },
