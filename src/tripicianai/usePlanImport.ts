...
import { importPlan, IMPORT_MAX_IMAGES, TripicianAIRequestError } from './tripicianAIService';
import { downscaleImage, DownscaleError } from '../utils/downscaleImage';
import { createTripAndOpen } from './createTripAndOpen';
import { composeImportantNotes, composeImportedExtras } from '../pages/CreateTripPage/planSeed';
    ...
    try {
      ...
      await createTripAndOpen({
        ...
        state: {
          planSeed: {
            stops: plan.stops,
            // Things to bring, a budget and costs have no section of their own now, so they join the notes.
            importantNotes: composeImportantNotes(plan.importantNotes, [
              ...plan.unplaced,
              ...composeImportedExtras(plan.checklist),
            ]),
          },
        },
        payload: {
      ...
      if (err instanceof TripicianAIRequestError) {
        setError(err.status === 402 ? 'You are out of TripicianAI credits.' : err.message);
      } else {
