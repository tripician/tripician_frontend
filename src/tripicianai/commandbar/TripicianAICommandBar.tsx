...
import { useAskTripicianAI } from './useAskTripicianAI';

const MotionForm = motion.create('form');

const TripicianAICommandBar: React.FC = () => {
  const { pathname } = useLocation();
  const bar = useCommandBar();
  const ask = useAskTripicianAI(bar.token);
  ...
  const submit = async () => {
    ...
    // Every mode needs an account: TripicianAIController is [Authorize] throughout.
    if (!bar.token) {
      bar.requireAuth({
        reason: 'Sign in and TripicianAI will pick this up where you left it.',
    ...
    <>
      ...
      <Box
        ...
        sx={{
           ...
           * to TripicianAI on every destination the bar can reach.
           */
          position: 'fixed',
          zIndex: 1250,
          left: 16,
                    ...
                    >
                      <motion.div variants={reduceMotion ? undefined : chipReveal}>
                        <SegmentedControl
                          ...
                          size="small"
                          aria-label="What TripicianAI should do"
                        />
                      </motion.div>
                    </motion.div>
                  </Box>
...
export default TripicianAICommandBar;
