...
import { IconCompass, IconFeather, IconRoute } from '@tabler/icons-react';
import TripicianAIOrb from '../TripicianAIOrb';
import { CREDIT_COSTS } from '../tripicianAIService';
...
export const COMMAND_MODES: Record<CommandMode, CommandModeSpec> = {
  auto: {
    ...
    acceptsImages: true,
    cost: null,
    contract: () => 'TripicianAI decides · nothing runs yet',
  },
  plan: {
    id: 'plan',
    label: 'Plan',
 ...
 *
 * /posts and /profile were added when the TripicianAI tab left the bottom bar. They
 * are bottom-bar destinations, and without them the only way to TripicianAI there was
 ...
 */
export const COMMAND_BAR_ROUTES = ['/search', '/stories', '/trips', '/templates', '/crew', '/posts', '/profile'];

/**
 * The root is the board, so the dock belongs there. It is matched separately
 * rather than added to the list above, because the prefix rule would turn a bare
 * '/' into "every route on the site".
 *
 * navConfig.test asserts every nav item's path satisfies this, so the three
 * remaining items all have to stay reachable here.
 */
